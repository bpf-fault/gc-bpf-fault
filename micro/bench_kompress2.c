...
int main(int argc, char **argv)
{
	...
	/* pad by one scratch chunk so the handler's bulk read near the end of
	 * from-space does not run past the mapping (real GC clamps to region). */
	size_t from_bytes = ((total * 8 + 4096 * 8 + ps - 1) & ~(ps - 1));
	...
	if (gc_kompress2_bpf__load(skel)) { fprintf(stderr, "load failed (root? memlock?)\n"); return 1; }
	...
	/* flat forward table: live word -> its post-compact ADDRESS */
	for (size_t w = 0; w < total; w++)
		if (liv[w])
			skel->arena->fwd[w] = (uint64_t)to + (uint64_t)newidx[w] * 8;
	...
	memset((char *)from + total * 8, 0, from_bytes - total * 8); /* resident padding */
	link = bpf_map__attach_fault_ops(skel->maps.gc_kompress2, to, to_bytes, 0);
	...
	printf("kompress2: heap_words=%zu objs=%zu live_words=%zu to=%zuMB "
	...
	printf("  in-kernel: faults=%llu words=%llu refs_forwarded=%llu  "
	       "(probe_read from-space -> scratch, in-kernel compaction)\n",
	       (unsigned long long)skel->bss->kc_faults,
	       (unsigned long long)skel->bss->kc_words,
	       (unsigned long long)skel->bss->kc_refs);
	fprintf(stderr, "  prefail=%llu chunkfull=%llu\n", (unsigned long long)skel->bss->kc_prefail, (unsigned long long)skel->bss->kc_chunkfull);
	printf("%s\n", bad ? "FAIL" : "PASS");

	bpf_link__destroy(link);
	gc_kompress2_bpf__destroy(skel);
	return bad ? 1 : 0;
}
